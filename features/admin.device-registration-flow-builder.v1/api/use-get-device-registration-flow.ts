...
import { DeviceRegistrationFlow } from "../models/flow";
 ...
 * - `https://{serverUrl}/t/{tenantDomain}/api/server/v1/device-registration-flow`
 ...
 */
const useGetDeviceRegistrationFlow = <Data = any, Error = RequestErrorInterface>(
    ...
    const requestConfig: RequestConfigInterface = {
        ...
        url: `${store.getState().config.endpoints.deviceRegistrationFlow}?flowType=${
            flowType || DeviceRegistrationFlowConstants.DEVICE_REGISTRATION_FLOW_TYPE
        }`
    };
    ...
    const migratedData: Data = useMemo(() => {
        ...
        try {
            const flow: DeviceRegistrationFlow = rawData as unknown as DeviceRegistrationFlow;
            const migratedFlow: DeviceRegistrationFlow = endStepMigrator(flow);
...
export default useGetDeviceRegistrationFlow;
