...
import DeviceRegistrationFlowAILoader from "./ai-device-registration-flow-generation-loader";
import StaticStepFactory from "./resources/steps/static-step-factory";
import StepFactory from "./resources/steps/step-factory";
import useAIDeviceRegistrationFlowGenerationResult from "../api/use-ai-device-registration-flow-result";
import useGetDeviceRegistrationFlow from "../api/use-get-device-registration-flow";
import useGetDeviceRegistrationFlowBuilderResources from "../api/use-get-device-registration-flow-builder-resources";
import { REGISTRATION_FLOW_AI_PROMPT_HISTORY_PREFERENCE_KEY } from "../constants/device-registration-flow-ai-constants";
import DeviceRegistrationFlowExecutorConstants from "../constants/device-registration-flow-executor-constants";
import useAIGeneratedDeviceRegistrationFlow from "../hooks/use-ai-generated-device-registration-flow";
import useDefaultFlow from "../hooks/use-default-flow";
import useGenerateDeviceRegistrationFlow, {
    UseGenerateDeviceRegistrationFlowFunction
} from "../hooks/use-generate-device-registration-flow";
import { RegistrationStaticStepTypes } from "../models/flow";

/**
 * Props interface of {@link DeviceRegistrationFlowBuilderCore}
 */
type DeviceRegistrationFlowBuilderCorePropsInterface = IdentifiableComponentInterface;
interface DeviceRegistrationFlowBuilderCoreExtendedPropsInterface extends DeviceRegistrationFlowBuilderCorePropsInterface {
    flowType?: FlowTypes;
}
 ...
 */
const DeviceRegistrationFlowBuilderCore: FunctionComponent<DeviceRegistrationFlowBuilderCoreExtendedPropsInterface> = ({
    "data-componentid": componentId = "device-registration-flow-builder-core",
    flowType = FlowTypes.REGISTRATION,
    ...rest
}: DeviceRegistrationFlowBuilderCoreExtendedPropsInterface): ReactElement => {
    ...
    const {
        ...
        setAIGeneratedFlow,
        aiGeneratedFlow
    } = useAIGeneratedDeviceRegistrationFlow();

    const {
        data: flowData,
        error: flowError
    } = useAIDeviceRegistrationFlowGenerationResult(operationId, flowGenerationCompleted);

    const generateAIDeviceRegistrationFlow: UseGenerateDeviceRegistrationFlowFunction = useGenerateDeviceRegistrationFlow();

    const {
        data: deviceRegistrationFlow,
        mutate: mutateDeviceRegistrationFlow,
        error: deviceRegistrationFlowFetchRequestError,
        isLoading: isDeviceRegistrationFlowFetchRequestLoading,
        isValidating: isDeviceRegistrationFlowFetchRequestValidating
    } = useGetDeviceRegistrationFlow(flowType);

    const { data: resources } = useGetDeviceRegistrationFlowBuilderResources(flowType);
     ...
     */
    useEffect(() => {
        if (deviceRegistrationFlowFetchRequestError) {
            dispatch(
                addAlert({
    ...
    }, [ deviceRegistrationFlowFetchRequestError ]);

    const handleAIFlowGeneration = async () => {
        const traceID: string = uuidv4();

        await generateAIDeviceRegistrationFlow(userPrompt, traceID);
    ...
    const handleAIGeneratedFlow = (data: any) => {
        ...
        const aiGeneratedDeviceRegistrationFlow: any = {
            "category": "STARTER",
            "config": {
                data : data
            },
        ...
        setAIGeneratedFlow(aiGeneratedDeviceRegistrationFlow);
        setShowAIGenerationModal(false);
        setIsFlowGenerating(false);
    ...
    const generateEdges = (steps: Step[]): Edge[] => {
        ...
        const createEdgesForButtons = (step: Step, button: Element): Edge[] => {
            ...
            if (button.action?.next) {
                ...
                DeviceRegistrationFlowExecutorConstants.PASSWORD_PROVISIONING_EXECUTOR) {
     ...
     */
    useLayoutEffect(() => {
        if (!isDeviceRegistrationFlowFetchRequestLoading && !isDeviceRegistrationFlowFetchRequestValidating) {
            if (!isEmpty(deviceRegistrationFlow?.steps)) {
                const steps: Node[] = generateSteps(deviceRegistrationFlow.steps);
        ...
        deviceRegistrationFlow?.steps,
        isDeviceRegistrationFlowFetchRequestLoading,
        isDeviceRegistrationFlowFetchRequestValidating,
        initialNodes
    ]);
                            ...
                            if (formComponent.type === ElementTypes.Button) {
                                return {
                                    ...
                                    action: {
                                        ...
                                        executor: {
                                            name: DeviceRegistrationFlowExecutorConstants.PASSWORD_PROVISIONING_EXECUTOR
                                        },
                                        type: "EXECUTOR"
                                    }
                                };
                            ...
                            if (formComponent.type === ElementTypes.Button) {
                                return {
                                    ...
                                    action: {
                                        ...(formComponent?.action ?? {}),
                                        executor: {
                                            name: DeviceRegistrationFlowExecutorConstants.EMAIL_OTP_EXECUTOR
                                        },
                                        type: "EXECUTOR"
                                    }
                                };
    ...
    return (
        <>
            ...
            { isFlowGenerating && <DeviceRegistrationFlowAILoader /> }
        </>
    );
};

export default DeviceRegistrationFlowBuilderCore;
