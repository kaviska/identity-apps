...
const FlowCompletionProperties: FunctionComponent<FlowCompletionPropertiesPropsInterface> = ({
    ...
    const { data: deviceRegistrationFlowConfig } = useGetFlowConfig(FlowTypes.REGISTRATION);
    ...
    const configs: FlowCompletionConfigsInterface = !isEmpty(flowCompletionConfigs)
        ...
        : deviceRegistrationFlowConfig?.flowCompletionConfigs;

    return (
        <Stack gap={ 2 } data-componentid={ componentId }>
            <Typography>
                <Trans i18nKey="flows:deviceRegistrationFlow.steps.end.description">
            ...
            <Box sx={ { display: "flex", flexDirection: "column", gap: 1 } }>
                { metadata?.supportedFlowCompletionConfigs?.includes("isEmailVerificationEnabled") && (
                    <Box>
                        <FormControlLabel
                            label={ t("flows:deviceRegistrationFlow.steps.end.accountVerification.label") }
                            control={
                                (<Checkbox
                        ...
                        />
                        <FormHelperText>
                            { t("flows:deviceRegistrationFlow.steps.end.accountVerification.hint") }
                        </FormHelperText>
                        { !metadata?.workflowEnabled && (
                            <Alert severity="warning">
                                { t("flows:deviceRegistrationFlow.steps.end.accountVerification.warning") }
                            </Alert>
                        ) }
                    </Box>
                ) }
                { metadata?.supportedFlowCompletionConfigs?.includes("isAccountLockOnCreationEnabled") && (
                    <Box sx={ { display: "flex", flexDirection: "column", ml: 3 } }>
                        <FormControlLabel
                            label={ t("flows:deviceRegistrationFlow.steps.end.accountActivation.activateImmediately.label") }
                            control={
                                (<Checkbox
                        ...
                        />
                        <FormHelperText>
                            { t("flows:deviceRegistrationFlow.steps.end.accountActivation.activateImmediately.hint") }
                        </FormHelperText>
                    </Box>
                ) }
                { metadata?.supportedFlowCompletionConfigs?.includes("isAutoLoginEnabled") && (
                    <Box>
                        <FormControlLabel
                            label={ t("flows:deviceRegistrationFlow.steps.end.autoLogin.label") }
                            control={
                                (<Checkbox
                        ...
                        />
                        <FormHelperText>{ t("flows:deviceRegistrationFlow.steps.end.autoLogin.hint") }</FormHelperText>
                    </Box>
                ) }
                { metadata?.supportedFlowCompletionConfigs?.includes("isFlowCompletionNotificationEnabled") && (
                    <Box>
                        <FormControlLabel
                            label={ t("flows:deviceRegistrationFlow.steps.end.accountFlowCompletion.label") }
                            control={
                                (<Checkbox
                        ...
                        />
                        <FormHelperText>
                            { t("flows:deviceRegistrationFlow.steps.end.accountFlowCompletion.hint") }
                        </FormHelperText>
                    </Box>
                ) }
            </Box>
